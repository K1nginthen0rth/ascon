/*
 * MSVC-compatible version of key_schedule.h from GIFT-COFB opt32.
 *
 * REARRANGE_RKEY_0..3 use GCC statement expressions that also return a value
 * (used as: rkey[i] = REARRANGE_RKEY_0(rkey[i])).
 * We replace them with static inline functions that accept u32 by value,
 * apply the same SWAPMOVE transformations, and return the result.
 * Semantics are identical to the original.
 *
 * Original: gift-cofb/crypto_aead/giftcofb128v1/opt32/key_schedule.h
 */
#ifndef KEYSCHEDULE_H_
#define KEYSCHEDULE_H_

/* SWAPMOVE and u32 must already be defined (force-included via giftb128.h). */

static __inline u32 _gift_rearrange_rkey_0(u32 x) {
    u32 tmp;
...
static __inline u32 _gift_rearrange_rkey_1(u32 x) {
    u32 tmp;
...
static __inline u32 _gift_rearrange_rkey_2(u32 x) {
    u32 tmp;
...
static __inline u32 _gift_rearrange_rkey_3(u32 x) {
    u32 tmp;
...
#define REARRANGE_RKEY_0(x)  _gift_rearrange_rkey_0(x)
#define REARRANGE_RKEY_1(x)  _gift_rearrange_rkey_1(x)
#define REARRANGE_RKEY_2(x)  _gift_rearrange_rkey_2(x)
#define REARRANGE_RKEY_3(x)  _gift_rearrange_rkey_3(x)

/* The remaining macros are pure expressions — no GCC extensions needed. */
