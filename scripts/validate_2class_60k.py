"""
Validacao do dataset 2-classes 60k (Ascon + GIFT-COFB).

Sanity checks:
  1. Totais por algoritmo (30.000 cada, 60.000 total)
...
def main() -> None:
    ...
    total_ok = (counts.get("Ascon-AEAD128") == 30000
                and counts.get("GIFT-COFB")  == 30000
                and len(df) == 60000)
    ...
    # --- decrypt spot check ---
    spot_skipped_reason = None
    if KEYS_JSON.exists() and NONCES_JSON.exists() and PT_PARQUET.exists():
        ...
        spot_skipped_reason = (
            ...
            f"(KEYS_JSON={KEYS_JSON.exists()}, NONCES_JSON={NONCES_JSON.exists()}, "
            f"PT_PARQUET={PT_PARQUET.exists()}) - decrypt spot check pulado."
        )
        print(f"  decrypt spot check: PULADO ({spot_skipped_reason})")
        spot = {"skipped": True, "reason": spot_skipped_reason}
        spot_ok = True  # nao bloqueia o veredicto quando pulado por falta de dados
