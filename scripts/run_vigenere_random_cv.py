"""
Experimento 2-classes: Vigenere-XOR vs PRNG-Random (64 KB).

Dataset  : vigenere_vs_random_v1 (60k amostras, PT=64KB, 300 chaves)
Classes  : Vigenere-XOR, PRNG-Random
...
Modelos  : RF(500) . SVM-RBF (C=10, gamma=scale) . LinearSVC . XGBoost(500)

Hipotese: Vigenere tem estrutura periodica (XOR 4 bytes) detectavel contra
bytes puramente aleatorios -> acuracia esperada proxima de 1.0 (controle
positivo extremo).

Saidas em reports/vigenere_vs_random_v1_cv/:
  cv_results.json
  final_results.json
  comparison_table.md
  mcnemar_table.md
...
Uso:
    python scripts/run_vigenere_random_cv.py
    python scripts/run_vigenere_random_cv.py --skip-cv    # reutiliza cache CV existente
    python scripts/run_vigenere_random_cv.py --skip-all   # reutiliza cache CV e final
"""
...
VIG_FEAT_PARQUET = DATA_DIR / "control_vigenere_64k_v1_features.parquet"
...
def _build_models() -> dict:
    ...
    return {
        ...
        "SVM": SVC(
            ...
            class_weight="balanced", random_state=s, probability=True,
        ),
        "LinearSVC": LinearSVC(
...
def ensure_features() -> pd.DataFrame:
    ...
    if VIG_FEAT_PARQUET.exists():
        # Reutiliza features Vigenere-XOR ja extraidas, extrai so PRNG-Random
        print(f"  Reutilizando features Vigenere-XOR de {VIG_FEAT_PARQUET.name} ...")
        raw_ids = pd.read_parquet(RAW_PARQUET, columns=["sample_id", "algorithm"])
        vig_ids = set(raw_ids.loc[raw_ids["algorithm"] == "Vigenere-XOR", "sample_id"])

        vig_feat = pd.read_parquet(VIG_FEAT_PARQUET)
        vig_feat = vig_feat[vig_feat["sample_id"].isin(vig_ids)].copy()
        print(f"    {len(vig_feat):,} amostras Vigenere-XOR reutilizadas")

        INTERIM_DIR.mkdir(parents=True, exist_ok=True)
        tmp_path = INTERIM_DIR / f"{DATASET_ID}_random_only_tmp.parquet"
        rand_raw = pd.read_parquet(RAW_PARQUET)
        rand_raw = rand_raw[rand_raw["algorithm"] == "PRNG-Random"]
        rand_raw.to_parquet(tmp_path, index=False)
        print(f"    Extraindo features PRNG-Random ({len(rand_raw):,} amostras) ...")

        extractor = CiphertextFeatureExtractor()
        rand_feat = extractor.extract_dataset(tmp_path, n_jobs=-1)
        tmp_path.unlink(missing_ok=True)

        df = pd.concat([vig_feat, rand_feat], ignore_index=True)
    else:
        print(f"  Extraindo features de {RAW_PARQUET.name}  (alguns minutos) ...")
        extractor = CiphertextFeatureExtractor()
        df = extractor.extract_dataset(RAW_PARQUET, n_jobs=-1)
...
def main() -> None:
    print(f"\n{'='*70}")
    print(f"  Experimento 2-classes: Vigenere-XOR vs PRNG-Random (64 KB)")
    ...
    if not RAW_PARQUET.exists():
        print(f"  ERRO: dataset nao encontrado: {RAW_PARQUET}")
        print(f"  Execute primeiro: python scripts/generate_vigenere_random_dataset.py")
        sys.exit(1)
    ...
    skip_all  = "--skip-all" in sys.argv

    if cv_cache.exists() and (skip_cv or skip_all):
        print(f"\n  [--skip-cv/--skip-all] Carregando cache CV de {cv_cache.name}")
    ...
    if final_cache.exists() and skip_all:
        print(f"\n  [--skip-all] Carregando cache final de {final_cache.name}")
        with final_cache.open("rb") as f:
            predictions, final_data = pickle.load(f)
    elif final_cache.exists():
    ...
    (REPORTS_DIR / "comparison_table.md").write_text(
        f"# Vigenere-XOR vs PRNG-Random (64 KB) "
        f"— 80/20 key-holdout + {N_FOLDS}-fold CV\n\n"
        f"Baseline acaso (1/2) = 0.5000\n\n{md_table}\n",
        encoding="utf-8",
    )
