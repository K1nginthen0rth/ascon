"""
Experimento central 60k — Ascon-AEAD128 vs GIFT-COFB
com 5-fold GroupKFold CV dentro do bloco treino+validação.

Dataset  : keyholdout_2class_60k_v1 (60k amostras, PT=64KB, 300 chaves)
Split    : 240 chaves trainval (80%) / 60 chaves test holdout (20%)
CV       : 5-fold GroupKFold estratificado por chave
             -> 192 chaves treino / 48 chaves val por fold
Feature selection: roda DENTRO de cada fold (regra de ouro anti-vazamento)
Modelos  : RF(500) · SVM-RBF (GridSearchCV 3-fold sobre C×gamma) · LinearSVC · XGBoost(500)

Seeds    : modelos=7, bootstrap=42, FS=13

Saídas em reports/keyholdout_2class_60k_v1_cv/:
  cv_results.json       — F1 por fold e modelo + estabilidade Jaccard
  final_results.json    — métricas finais no test holdout (bootstrap 1000×)
  comparison_table.md   — tabela de resultados
  mcnemar_table.md      — McNemar com correção de continuidade
  confusion_matrices/   — PNGs por modelo

Uso:
    python scripts/run_experiment_60k_cv.py
    python scripts/run_experiment_60k_cv.py --skip-cv   # usa cache de CV se existir
"""
...
# Modelos que precisam de StandardScaler antes do fit
_NEEDS_SCALING = {"LinearSVC", "SVM"}

# Grid de hiperparâmetros para SVM-RBF
SVM_PARAM_GRID = {
    "C":     [0.1, 1, 10, 100],
...
def _get_proba(model, X: np.ndarray) -> np.ndarray | None:
    """Extrai probabilidades do modelo; usa softmax do decision_function se necessário."""
...
# ---------------------------------------------------------------------------
# Passo 0 — extração de features (com cache)
# ---------------------------------------------------------------------------

def ensure_features() -> pd.DataFrame:
    """Extrai features se não existir cache, senão carrega do parquet."""
    if FEAT_PARQUET.exists():
        print(f"  Features já extraídas: {FEAT_PARQUET.name}")
        return pd.read_parquet(FEAT_PARQUET)
    print(f"  Extraindo features de {RAW_PARQUET.name}  (isso leva alguns minutos)...")
...
# ---------------------------------------------------------------------------
# Passo 1 — carregar e preparar splits (80/20)
# ---------------------------------------------------------------------------

def prepare_data() -> tuple[pd.DataFrame, pd.DataFrame, list[str], dict[str, int]]:
    """
    Carrega features e define trainval (train+val originais = 240 chaves)
    e test holdout (60 chaves).

    O split file usa a nomenclatura antiga 60/20/20 (train/val/test).
    Aqui fundimos train+val em um bloco trainval único conforme o protocolo 80/20.

    Returns:
        (trainval_df, test_df, classes, label_map)
    """
    feat_df = ensure_features()

    # Enriquecer com split e key_id do parquet original se necessário
    ...
    if feat_df["split"].isna().any():
        raise ValueError("Amostras sem coluna 'split' após merge — verifique sample_id.")

    # Protocolo 80/20: train + val -> trainval
    ...
    # Verificar key-holdout estrito
    tv_keys  = set(trainval_df["key_id"].unique())
    tst_keys = set(test_df["key_id"].unique())
    overlap  = tv_keys & tst_keys
    if overlap:
        raise ValueError(f"VAZAMENTO detectado: chaves em trainval∩test = {overlap}")
...
def run_cv(
    ...
    """
    Executa 5-fold GroupKFold CV sobre o bloco trainval.

    Cada fold:
      - Grupos = key_id -> nenhuma chave aparece em treino e val do mesmo fold
      - Feature selection fit exclusivamente no treino do fold
      - StandardScaler fit exclusivamente no treino do fold (para LinearSVC)
      - Avaliação no val do fold sem bootstrap (F1-macro + BalAcc raw)

    Returns:
        dict com métricas por fold e estabilidade de features (Jaccard).
    """
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(gkf.split(X_all, y_all, groups)):
        ...
        for name, mdl in fold_models.items():
            ...
            best_params = (
                mdl.best_params_ if hasattr(mdl, "best_params_") else None
            )
            params_str = (
                f"  best={best_params}" if best_params else ""
            )
    ...
    # Estabilidade de features: Jaccard entre todos os pares de folds
    pairs_jac = []
...
# ---------------------------------------------------------------------------
# Passo 3 — modelo final (trainval completo -> test holdout)
# ---------------------------------------------------------------------------
...
def run_final(
    ...
    """
    Treina no trainval completo (240 chaves) e avalia no test holdout (60 chaves).
    Feature selection e scaler ajustados apenas no trainval.
    Bootstrap 1000× para IC 95% e McNemar pareado.

    Returns:
        (predictions_dict, final_data_dict)
    """
    print(f"\n--- Modelo Final: trainval completo ({len(trainval_df):,}) -> test ({len(test_df):,}) ---")
...
def _save_confusion_matrices(predictions: dict, label_names: list[str]) -> None:
    try:
        ...
        print("  matplotlib indisponível — pulando CMs em PNG.")
        return
    ...
    for name, payload in predictions.items():
        ...
        ax.set_title(f"Confusão — {name}", fontsize=10)
...
def main() -> None:
    print(f"\n{'='*70}")
    print(f"  Experimento 60k — Ascon-AEAD128 vs GIFT-COFB")
    ...
    assert len(feat_cols) == 307, f"Esperava 307 features, obteve {len(feat_cols)}"
    print(f"  Features: {len(feat_cols)}")

    # --- CV (com cache para retomar após crash) ---
    ...
    print(f"  Estabilidade features (Jaccard médio cross-fold): {stab:.4f}")

    # --- Modelo final (com cache) ---
    ...
    # McNemar entre todos os pares (com correção de continuidade + Bonferroni)
    pairs = list(combinations(predictions.keys(), 2))
    mcnemar_rows: list[list[str]] = []
    pair_results: list[dict] = []
    alpha_bonferroni = 0.05 / len(pairs) if pairs else 0.05
    for a, b in pairs:
    ...
    mcnemar_md = _format_table(
        mcnemar_rows,
        [f"Comparação (alpha={alpha_bonferroni:.4f})", "estatística", "p-value", "Significativo?"],
    )
        ...
        json.dumps(
            {
                **final_data,
                "y_true": y_tst.tolist(),
                "mcnemar_alpha_bonferroni": alpha_bonferroni,
    ...
    (REPORTS_DIR / "comparison_table.md").write_text(
        f"# Ascon-AEAD128 vs GIFT-COFB — 60k (80/20 key-holdout + {N_FOLDS}-fold CV)\n\n"
        f"Baseline acaso = 0.5000\n\n{md_table}\n",
        encoding="utf-8",
    )
    (REPORTS_DIR / "mcnemar_table.md").write_text(
        f"# Teste de McNemar (correção de continuidade + Bonferroni, alpha={alpha_bonferroni:.4f})\n\n"
        f"{mcnemar_md}\n",
        encoding="utf-8",
    )
