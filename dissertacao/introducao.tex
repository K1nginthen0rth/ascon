...
Do ponto de vista teórico, cifras modernas, leves ou convencionais, devem produzir criptogramas indistinguíveis de ruído uniforme para qualquer adversário eficiente, resistindo a diferentes modelos de ataque: texto cifrado apenas (COA), texto conhecido (KPA), texto escolhido (CPA) e, em cenários mais fortes, texto cifrado escolhido (CCA). Em todos os casos, a saída deve ser indistinguível do aleatório e não pode revelar informação sobre o texto claro sem conhecimento da chave, o que se alinha ao princípio clássico de Shannon de que um criptograma não deve revelar informação sobre o texto em claro sem a chave \cite{ShannonWeaver1949MTC, Rukhin2010NIST80022}. Formalmente, essa propriedade é capturada pela noção de indistinguibilidade sob ataque de texto em claro escolhido (IND-CPA), que exige que nenhum adversário eficiente consiga distinguir, com vantagem não negligenciável, criptogramas associados a mensagens de mesmo comprimento, introduzida no arcabouço de \citeonline{GoldwasserMicali1984PE}.
...
Pesquisadores propuseram que algoritmos leves poderiam deixar assinaturas sutis em seus padrões de saída, permitindo classificação a partir de amostras de texto cifrado. Trabalhos como \cite{Yuan2023HRFLR} e \cite{Kopal2020CiphersNeurons} mostram que modelos de aprendizado de máquina podem, em condições de laboratório e controladas, distinguir criptogramas gerados por diferentes algoritmos LWC, sugerindo que aspectos na construção podem induzir distribuições de bytes ligeiramente distintas.
...
A partir desse contexto, coloca-se a seguinte pergunta de pesquisa: é possível, a partir exclusivamente de criptogramas, identificar o algoritmo de criptografia leve que os gerou, utilizando modelos de aprendizado de máquina, com desempenho estatisticamente superior ao acaso? O problema é enquadrado como atribuição algorítmica por classificação, sem buscar recuperação de chave, revelação de texto em claro ou identificação de fragilidades criptográficas diretas \cite{Turan2023NISTLWCFinal}.
...
O problema aqui tratado é estritamente de atribuição algorítmica: considerando
apenas criptogramas produzidos por algoritmos LWC modernos (cifras leves com
alvo de 128 bits), investiga-se se é possível inferir qual algoritmo os gerou
...
Dado um conjunto de algoritmos $\mathcal{A}=\{A_1,\ldots,A_k\}$ e um conjunto
...
chave~\cite{ShannonWeaver1949MTC}. No cenário computacional moderno, a noção de
...
arcabouço de Goldwasser--Micali, na qual nenhum adversário eficiente deve
distinguir, com vantagem não negligenciável, criptogramas associados a mensagens
de mesmo tamanho escolhidas pelo
adversário~\cite{GoldwasserMicali1984PE}. A expectativa teórica é que
criptogramas se comportem como ruído para adversários eficientes, dado um
esquema corretamente projetado e utilizado.

Ainda assim, se algoritmos LWC padronizados puderem ser consistentemente
reconhecidos por ML sob um protocolo experimental controlado e explicitamente
...
Esta dissertação tem como objetivo identificar o algoritmo de criptografia leve (LWC) gerador de um criptograma observado, considerando exclusivamente o modelo de acesso somente a criptogramas.
